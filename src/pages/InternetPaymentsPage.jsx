...
const PROVIDERS = [
  { id: 'citynet', name: 'CityNet', color: '#0e5af1', bg: '#eef2ff' },
  { id: 'katv', name: 'KATV1 İnternet', color: '#10b981', bg: '#ecfdf5' },
  { id: 'baktelecom', name: 'Baktelecom', color: '#0284c7', bg: '#f0f9ff' },
  { id: 'connect', name: 'Connect TV & Net', color: '#8b5cf6', bg: '#f5f3ff' },
];

const PRESET_AMOUNTS = [15, 18, 20, 25, 30];

const InternetPaymentsPage = () => {
  ...
  // 1. Səhifə yüklənəndə Kartları və Hesabları paralel gətirmək
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Aktiv kartları filtr edib hesabdakı balansla cütləşdiririk
  ...
  // 2. İnternet Ödənişinin İcrası (Withdraw)
  const handlePayment = async (e) => {
    e.preventDefault();
    const cleanContract = contractNo.trim();
    ...
    if (!cleanContract) {
      setError('Müqavilə və ya abunəçi kodunu daxil edin.');
      return;
    }
    ...
    <div style={pageStyle}>
      ...
      <div style={headerStyle}>
        ...
        <h3 style={{ fontSize: '1.15rem', fontWeight: '700', color: '#212529', margin: 0 }}>
          {selectedProvider ? selectedProvider.name : 'İnternet Ödənişi'}
        </h3>
      </div>
      ...
      {step === 'select' && (
        <div>
          <p style={subTitleStyle}>Provayderi seçin</p>
          {PROVIDERS.map(p => (
            <div key={p.id} onClick={() => handleSelectProvider(p)} style={itemCardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ ...iconBox, backgroundColor: p.bg }}>
                  <Wifi size={22} color={p.color} />
                </div>
      ...
      {/* ================= STEP 2: MÜQAVİLƏ, MƏBLƏĞ VƏ KART SEÇİMİ ================= */}
      {step === 'details' && (
        <form onSubmit={handlePayment} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          
          {/* Müqavilə Kodu Inputu */}
          <div>
            <label style={labelStyle}>Müqavilə / Abunəçi Kodu</label>
            <input 
              type="text" 
              placeholder="Məs: CN-10293" 
              value={contractNo} 
          ...
          <div>
            <label style={labelStyle}>Aylıq Ödəniş Məbləği (₼)</label>
            ...
            <input 
              type="number" 
              placeholder="Fərqli məbləğ yazın" 
              value={amount} 
      ...
      {step === 'success' && (
        <div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          <h4 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#212529' }}>Ödəniş Qəbul Edildi!</h4>
          <p style={{ color: '#6c757d', margin: '0.5rem 0 1.5rem' }}>
            <b>{contractNo}</b> müqaviləsinə <b>{amount} ₼</b> ödənildi.
          </p>
...
export default InternetPaymentsPage;