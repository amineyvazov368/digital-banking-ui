...
const CREDIT_PROVIDERS = [
  { id: 'kapital_loan', name: 'Kapital Bank Kredit', color: '#ef4444', bg: '#fef2f2' },
  { id: 'abb_loan', name: 'ABB Kredit', color: '#0e5af1', bg: '#eef2ff' },
  { id: 'unibank_loan', name: 'Unibank Kredit', color: '#f97316', bg: '#fff7ed' },
  { id: 'leasing', name: 'Lizinq Ödənişləri', color: '#64748b', bg: '#f8fafc' },
];

const PRESET_AMOUNTS = [50, 100, 200, 500];

const CreditPaymentsPage = () => {
  ...
  // 2. Kredit Ödənişinin İcrası (Withdraw)
  const handlePayment = async (e) => {
    ...
    if (!contractNo.trim()) {
      setError('Kredit müqavilə nömrəsini daxil edin.');
      return;
    }
    if (!finCode.trim()) {
    ...
    try {
      ...
      setLoading(false);
      setError(err?.response?.data?.message || 'Kredit ödənişi icra olunmadı. Yenidən cəhd edin.');
    }
  };
    ...
    <div style={pageStyle}>
      ...
      <div style={headerStyle}>
        ...
        <h3 style={{ fontSize: '1.15rem', fontWeight: '700', color: '#212529', margin: 0 }}>
          {selectedProvider ? selectedProvider.name : 'Kredit Ödənişi'}
        </h3>
      </div>

      {/* ================= STEP 1: TƏŞKİLAT SEÇİMİ ================= */}
      {step === 'select' && (
        <div>
          <p style={subTitleStyle}>Təşkilatı seçin</p>
            ...
            <div key={p.id} onClick={() => handleSelect(p)} style={itemCardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ ...iconBox, backgroundColor: p.bg }}>
                  <ShieldCheck size={22} color={p.color} />
                </div>
      ...
      {step === 'details' && (
        <form onSubmit={handlePayment} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            <label style={labelStyle}>Kredit Müqavilə №</label>
            <input 
              type="text" 
              placeholder="Məs: K-849201" 
              value={contractNo} 
          ...
          >
            {loading ? 'İcra olunur...' : 'Krediti Ödə'}
          </button>
        </form>
      )}
      ...
      {step === 'success' && (
        <div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          <h4 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#212529' }}>Kredit Ödənişi Tamamlandı!</h4>
          <p style={{ color: '#6c757d', margin: '0.5rem 0 1.5rem' }}>
            <b>{contractNo}</b> müqaviləsinə <b>{amount} ₼</b> məbləğində ödəniş uğurla icra olundu.
          </p>
