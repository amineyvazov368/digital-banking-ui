...
const TV_PROVIDERS = [
  { id: 'katv1', name: 'KATV1', color: '#10b981', bg: '#ecfdf5' },
  { id: 'aile_tv', name: 'Ailə TV', color: '#ec4899', bg: '#fdf2f8' },
  { id: 'atv_plus', name: 'ATV Plus', color: '#f59e0b', bg: '#fffbeb' },
  { id: 'connect_tv', name: 'Connect TV', color: '#8b5cf6', bg: '#f5f3ff' },
];

const PRESET_AMOUNTS = [10, 12, 15, 20];

const TvPaymentsPage = () => {
  ...
  // 1. Aktiv Kartları və Hesab Balanslarını Dinamik Yükləmək
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Aktiv kartları süzüb aid olduğu hesabın balansı ilə birləşdiririk
  ...
  // 2. Kabel TV Ödənişinin Backend-ə Göndərilməsi (Withdraw)
  const handlePayment = async (e) => {
    e.preventDefault();
    const cleanSubscriberNo = subscriberNo.trim();
    ...
    if (!cleanSubscriberNo) {
      setError('Abunəçi və ya smart kart nömrəsini daxil edin.');
      return;
    }
    ...
    <div style={pageStyle}>
      ...
      <div style={headerStyle}>
        ...
        <h3 style={{ fontSize: '1.15rem', fontWeight: '700', color: '#212529', margin: 0 }}>
          {selectedProvider ? selectedProvider.name : 'Kabel TV Ödənişi'}
        </h3>
      </div>

      {/* ================= STEP 1: TV PROVAYDERİ SEÇİMİ ================= */}
      {step === 'select' && (
        <div>
          <p style={subTitleStyle}>Kabel TV provayderini seçin</p>
          {TV_PROVIDERS.map(p => (
            <div key={p.id} onClick={() => handleSelect(p)} style={itemCardStyle}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ ...iconBox, backgroundColor: p.bg }}>
                  <Tv size={22} color={p.color} />
                </div>
      ...
      {/* ================= STEP 2: ABUNƏÇİ KODU, MƏBLƏĞ VƏ KART SEÇİMİ ================= */}
      {step === 'details' && (
        <form onSubmit={handlePayment} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          
          {/* Abunəçi Kodu Inputu */}
          <div>
            <label style={labelStyle}>Abunəçi Kodu / Smart Kart Nömrəsi</label>
            <input 
              type="text" 
              placeholder="Məs: 102938" 
              value={subscriberNo} 
          ...
          <div>
            ...
            <input 
              type="number" 
              placeholder="Fərqli məbləğ daxil edin" 
              value={amount} 
      ...
      {step === 'success' && (
        <div style={{ textAlign: 'center', padding: '2rem 0' }}>
          ...
          <h4 style={{ fontSize: '1.2rem', fontWeight: 'bold', color: '#212529' }}>Ödəniş Tamamlandı!</h4>
          <p style={{ color: '#6c757d', margin: '0.5rem 0 1.5rem' }}>
            <b>{subscriberNo}</b> hesabına <b>{amount} ₼</b> ödənildi.
          </p>
